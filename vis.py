...
def draw_tsne(netF,netB,netC, src_data_loader, tgt_data_loader, res_dir, idx, title, separate=True):
    # model.eval()
    ...
    count = 0
    with torch.no_grad():
        iter_test = iter(src_data_loader)
        for i in range(len(src_data_loader)):
            ...
            embedding = netB(netF(inputs),t=0)
            preds = netC(netB(netF(inputs)))
    ...
    with torch.no_grad():
        ...
        iter_test = iter(tgt_data_loader)
        for i in range(len(tgt_data_loader)):
            data = iter_test.next()
            inputs = data[0]
            labels = data[1]
            inputs = inputs.cuda()
...
def do_draw_confusion_matrix(y, y_pred, res_dir, epoch, title):
    ...
    if (len(classes) == 3):
        ...
        # paderborn 15
        labellist = ['K001', 'K002', 'K003', 'K004', 'K005', 'KA04', 'KA15', 'KA16', 'KA22', 'KA30', 'KI04', 'KI14',
                     'KI16', 'KI18', 'KI21']
        xlabellist = ['K001', 'K002', 'K003', 'K004', 'K005', 'KA04', 'KA15', 'KA16', 'KA22', 'KA30', 'KI04', 'KI14',
                      'KI16', 'KI18', 'KI21', 'Acc']
        ylabellist = ['K001', 'K002', 'K003', 'K004', 'K005', 'KA04', 'KA15', 'KA16', 'KA22', 'KA30', 'KI04', 'KI14',
                      'KI16', 'KI18', 'KI21', 'Recall']
    elif (len(classes) == 10):
        # cwru deonly
    ...
    sns.heatmap(confusion_matrix_result, xticklabels=xlabellist, yticklabels=ylabellist, cmap=plt.cm.Blues, annot=True,
                fmt="g", cbar=False, annot_kws={"size": 10})
...
def draw_confusion_matrix(netF,netB, netC, loader, res_dir, epoch, title):
    netF.eval()
    netB.eval()
    netC.eval()
    print('----------------------------draw----------------------------------')
    ...
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            ...
            y.append(labels)
            preds = netC(netB(netF(inputs),t=1))
            pred_cls = preds.data.max(1)[1]
